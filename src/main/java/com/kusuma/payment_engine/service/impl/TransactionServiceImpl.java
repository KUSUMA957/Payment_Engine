...
//		// AccountValidationUtil.validateAccountForTransactions(sender);
...
//				.senderAccount(sender).receiverAccount(receiver).amount(request.amount()).currency(sender.getCurrency())
//				.transactionType(TransactionType.TRANSFER).status(TransactionStatus.SUCCESS)
//				.description(request.description()).processedAt(LocalDateTime.now()).build();
...
//		// AccountValidationUtil.validateAccountForTransactions(account);
//		AccountValidationUtil.validateUserAndAccountForTransactions(user, account);
//		account.setBalance(account.getBalance().add(request.amount()));
//		accountRepository.save(account);
//		Transaction transaction = Transaction.builder().transactionReference(TransactionReferenceUtil.generate())
//				.receiverAccount(account).amount(request.amount()).currency(account.getCurrency())
//				.transactionType(TransactionType.DEPOSIT).status(TransactionStatus.SUCCESS)
//				.description(request.description()).processedAt(java.time.LocalDateTime.now()).build();
//		transaction = transactionRepository.save(transaction);
//		return mapToResponse(transaction);
//	}
//
...
//		accountRepository.save(account);
//		Transaction transaction = Transaction.builder().transactionReference(TransactionReferenceUtil.generate())
//				.senderAccount(account).amount(request.amount()).currency(account.getCurrency())
//				.transactionType(TransactionType.WITHDRAWAL).status(TransactionStatus.SUCCESS)
//				.description(request.description()).processedAt(java.time.LocalDateTime.now()).build();
//		transaction = transactionRepository.save(transaction);
//		return mapToResponse(transaction);
//	}
...
//			owner = transaction.getSenderAccount().getUser().getId().equals(loggedInUser.getId());
//		}
//		if (!owner && transaction.getReceiverAccount() != null) {
//			owner = transaction.getReceiverAccount().getUser().getId().equals(loggedInUser.getId());
//		}
//		if (!isAdmin && !owner) {
